...
async function fetchWithRetry(url, options = {}, retryCount = 0) {
  ...
  try {
    ...
    if (response.status === 429 && retryCount < MAX_RETRY_ATTEMPTS) {
      ...
      console.log(
        `收到 429 錯誤，${retryDelay}ms 後進行第 ${retryCount + 1} 次重試...`,
      );
    ...
    if (retryCount < MAX_RETRY_ATTEMPTS) {
      ...
      console.log(
        `網路錯誤，${retryDelay}ms 後進行第 ${retryCount + 1} 次重試...`,
        error,
      );
...
let isMobileView = false; // 是否為手機視圖
let isFilterPanelCollapsed = false; // 過濾面板是否收合
const MOBILE_BREAKPOINT = 576; // 手機斷點 (px)
...
const translations = {
  "zh-TW": {
    ...
    footer:
      ' 2025 TRC20 USDT 交易紀錄查詢 | 使用 <a href="https://picocss.com" target="_blank">Pico.css</a> | <a href="mailto:avhacker@gmail.com">avhacker@gmail.com</a>',
  ...
  en: {
    ...
    footer:
      ' 2025 TRC20 USDT Transaction Viewer | Powered by <a href="https://picocss.com" target="_blank">Pico.css</a> | <a href="mailto:avhacker@gmail.com">avhacker@gmail.com</a>',
    ...
    loadingLimitReached:
      "Reached default limit ({0} records). Click 'Continue' to load more",
    btnToday: "Today",
...
function applyTranslations() {
  ...
  document.getElementById("walletAddressLabel").textContent =
    t.walletAddressLabel;
  document.getElementById("energyLabel").textContent = t.energyLabel;
  document.getElementById("matchedTransactionsLabel").textContent =
    t.matchedTransactionsLabel;
  document.getElementById("totalReceivedLabel").textContent =
  ...
  document.getElementById("peerAddressFilter").placeholder =
    t.peerAddressPlaceholder;

  // 更新快速按鈕文字
  if (document.getElementById("btnToday"))
    document.getElementById("btnToday").textContent = t.btnToday;
  if (document.getElementById("btn3Days"))
    document.getElementById("btn3Days").textContent = t.btn3Days;
  if (document.getElementById("btnWeek"))
    document.getElementById("btnWeek").textContent = t.btnWeek;

  // 更新時間切換按鈕的 title
  if (document.getElementById("startTimeToggle"))
    document.getElementById("startTimeToggle").title = t.timeToggleTitle;
  if (document.getElementById("endTimeToggle"))
    document.getElementById("endTimeToggle").title = t.timeToggleTitle;

  // 更新地址過濾類型選項
  const peerAddressFilterTypeSelect = document.getElementById(
    "peerAddressFilterType",
  );
...
function updateURLParams() {
  ...
  const minAmount =
    parseFloat(document.getElementById("minAmount").value) || 1.0;
  const maxAmount = document.getElementById("maxAmount").value;
  const peerAddressFilter = document.getElementById("peerAddressFilter").value;
  const peerAddressFilterType = document.getElementById(
    "peerAddressFilterType",
  ).value;
  ...
  if (maxAmount) urlParams.set("max", maxAmount);
...
function loadURLParams() {
  ...
  document.getElementById("peerAddressFilter").value =
    urlParams.get("peer") || "";
  document.getElementById("peerAddressFilterType").value =
    urlParams.get("peerType") || "include";
  document.getElementById("actionFilter").value =
    urlParams.get("action") || "both";
...
function sortAndFilterTransactions() {
  ...
  const maxAmount = parseFloat(
    document.getElementById("maxAmount").value || null,
  );
  const peerAddressFilter =
    document.getElementById("peerAddressFilter").value.toLowerCase() || null;
  const peerAddressFilterType = document.getElementById(
    "peerAddressFilterType",
  ).value;
  ...
  let filteredTransactions = allTransactions.filter((tx) => {
    ...
    if (peerAddressFilter) {
      const addressIncludesPattern = tx.peerAddress
        .toLowerCase()
        .includes(peerAddressFilter);
...
function checkAndResumeLoading() {
  ...
  if (startTime) {
    ...
    if (startTime < minLoadedTimestamp) {
      console.log(
        `需要載入更多資料: 目標時間 ${new Date(startTime).toLocaleString()} < 目前最小時間 ${new Date(minLoadedTimestamp).toLocaleString()}`,
      );
...
function displayFilteredTransactions(transactions) {
  ...
  transactions.forEach((tx) => {
    ...
    transactionLink.textContent =
      tx.transactionId.substring(0, 6) +
      "..." +
    ...
    actionCell.className =
      tx.action === "received" ? "action-received" : "action-sent";

    const valueCell = row.insertCell();
    valueCell.textContent = tx.value.toLocaleString(undefined, {
    ...
    peerAddressLink.textContent =
      tx.peerAddress.substring(0, 6) +
      "..." +
      tx.peerAddress.substring(tx.peerAddress.length - 6);
  ...
  document.getElementById("totalReceived").textContent =
    window.filteredTotalReceived.toLocaleString(undefined, {
      minimumFractionDigits: 0,
      maximumFractionDigits: 6,
    });
  document.getElementById("totalSent").textContent =
    window.filteredTotalSent.toLocaleString(undefined, {
      minimumFractionDigits: 0,
      maximumFractionDigits: 6,
    });
}
...
async function fetchEnergy(address) {
  try {
    ...
    console.log(
      `能量資訊: 總能量限制 ${energyLimit}, 已使用 ${energyUsed}, 可用 ${energyAvailable}`,
    );
    ...
    while (shouldContinue && !isPaused) {
      ...
      if (startDateVal) {
        ...
        if (minLoadedTimestamp < startTime) {
          console.log(
            `已載入至設定的開始時間 (${startDateVal} ${startTimeVal})，暫停載入。`,
          );
...
window.onload = () => {
  ...
  if (address) {
    ...
    document.getElementById("energyBalance").innerHTML =
      '0 / <span style="color: var(--sent-color);">0</span>';

    fetchBalance(address)
      .then((balance) => {
        const formattedUsdt = balance.usdt.toLocaleString("en-US", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        });
        const formattedTrx = balance.trx.toLocaleString("en-US", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        });
  ...
  document
    ...
    .addEventListener("input", sortAndFilterTransactions);
  document
    .getElementById("maxAmount")
    .addEventListener("input", sortAndFilterTransactions);
  document
    .getElementById("peerAddressFilter")
    .addEventListener("input", sortAndFilterTransactions);
  document
    .getElementById("peerAddressFilterType")
    .addEventListener("change", sortAndFilterTransactions);
  document
  ...
  // 時間切換按鈕事件
  document.getElementById("startTimeToggle").onclick = function () {
    toggleTimeInput("start");
  };
  document.getElementById("endTimeToggle").onclick = function () {
    toggleTimeInput("end");
  };
  ...
  document.getElementById("btn3Days").onclick = function () {
    setQuickTimeRange(2);
  };
...
function updateWalletBalanceLabel(address) {
  ...
  if (address) {
    ...
    document.getElementById("walletBalanceLabel").textContent = formatString(
...
function toggleFilterPanel() {
  ...
  if (isFilterPanelCollapsed) {
    ...
    filterToggle.classList.remove("filter-panel__toggle--active");
  } else {
    filterContent.style.display = "flex";
    filterToggle.classList.add("filter-panel__toggle--active");
  }
}
...
function checkViewMode() {
  ...
  // 如果視圖模式變更，重新渲染交易列表
  if (wasMobile !== isMobileView && allTransactions.length > 0) {
    sortAndFilterTransactions();
  }
}
...
function renderTransactionCards(transactions) {
  ...
  transactions.forEach((tx) => {
    ...
    const actionText = isReceived ? t.typeReceived : t.typeSent;
    const peerLabel = isReceived ? t.fromLabel : t.toLabel;
    ...
    card.innerHTML = `
      <div class="transaction-card__header">
        <span class="transaction-card__time" data-timestamp="${tx.timestamp}"></span>
        <span class="badge badge--${tx.action}">${actionText}</span>
      </div>
      <div class="transaction-card__body">
        <span class="transaction-card__amount transaction-card__amount--${tx.action}">${formattedValue} USDT</span>
      </div>
      <div class="transaction-card__footer">
        <div class="transaction-card__peer">
          <span class="transaction-card__peer-label">${peerLabel}:</span>
          <a href="#" class="address-link" title="${tx.peerAddress}" onclick="document.querySelector('input[name=address]').value='${tx.peerAddress}';document.getElementById('queryButton').click();return false;">
            ${tx.peerAddress.substring(0, 6)}...${tx.peerAddress.substring(tx.peerAddress.length - 6)}
          </a>
        </div>
        <div class="transaction-card__actions">
          <span class="copy-icon" title="${translations[currentLanguage].copyTooltip}" onclick="copyToClipboard('${tx.peerAddress}', this, event)">
            ${getCopyIconSvg()}
          </span>
          <a href="https://tronscan.org/#/transaction/${tx.transactionId}" target="_blank" class="copy-icon" title="${translations[currentLanguage].tronscanTooltip}">
    ...
    // 更新時間顯示
    const timeCell = card.querySelector(".transaction-card__time");
    updateTimestampDisplay(timeCell, tx.timestamp);
  });
}
...
function updateFilterToggleText() {
  ...
  if (filterToggleText) {
    filterToggleText.textContent = translations[currentLanguage].filterToggle;
  }
}
